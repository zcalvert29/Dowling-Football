...
st.set_page_config(page_title="Go for 2 Bot", page_icon="🎯", layout="centered")
...
with st.sidebar:
    ...
    your_abbr = st.text_input("Your team", "DCHS").upper()[:4]
    opp_abbr = st.text_input("Opponent", "VHS").upper()[:4]
    your_score = st.number_input(
        f"{your_abbr} score (BEFORE the try — after the TD's 6 points)", 0, 99, 20
    )
    opp_score = st.number_input(f"{opp_abbr} score", 0, 99, 25)
